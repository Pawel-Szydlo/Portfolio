...
function FallbackVisual({ project, large }: { project: Project; large: boolean }) {
  const background = project.accent === "lime" ? "bg-[#d5f15c]" : project.accent === "blue" ? "bg-[#cbdcf4]" : "bg-[#f1d8c6]";
  return (
...
export function ProjectVisual({ project, large = false }: { project: Project; large?: boolean }) {
  ...
  return <div className="relative overflow-hidden rounded-2xl" role="region" aria-roledescription="carousel" aria-label={`${project.title} screenshots`}><div className={`relative ${large ? "aspect-[16/8]" : "aspect-[4/3]"}`}>{slides.map((slide, index) => <div key={slide.label} className={`absolute inset-0 transition-opacity duration-700 ${index === active ? "opacity-100" : "pointer-events-none opacity-0"}`} aria-hidden={index !== active}>{slide.src ? <Image src={slide.src} alt={slide.alt} fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" priority={index === 0} /> : <FallbackVisual project={project} large={large} />}<div className="absolute bottom-5 left-5 rounded-full bg-white/80 px-3 py-1 text-xs font-medium text-ink">{slide.label}</div></div>)}</div><div className="absolute bottom-5 right-5 flex gap-2"><button type="button" onClick={() => goTo(active - 1)} className="flex h-9 w-9 items-center justify-center rounded-full bg-white/85 text-ink transition hover:bg-white" aria-label="Previous image"><ChevronLeft size={16} /></button><button type="button" onClick={() => goTo(active + 1)} className="flex h-9 w-9 items-center justify-center rounded-full bg-white/85 text-ink transition hover:bg-white" aria-label="Next image"><ChevronRight size={16} /></button></div><div className="absolute left-5 top-5 flex gap-1.5" aria-label="Choose image">{slides.map((slide, index) => <button key={slide.label} type="button" onClick={() => goTo(index)} className={`h-1.5 rounded-full transition-all ${index === active ? "w-7 bg-ink" : "w-1.5 bg-ink/35"}`} aria-label={`Show ${slide.label}`} aria-current={index === active} />)}</div></div>;
}
